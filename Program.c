...
#define SWAP(type, array, x, y) type z=array[x]; array[x]=array[y]; array[y]=z; //макрос, мен€ющий местами элементы массива
...
int main(){
    FILE *file;
    file=fopen("file.txt", "w"); //открытие файла дл€ записи в него
    int length;
    float timeBubble, timeHeap;
    for(length=MIN_SIZE; length<=MAX_SIZE; length+=STEP){ //рассматривание работу алгоритмов дл€ массивов длиной от MIN_SIZE до MAX_SIZE
        int *massBubble=malloc(sizeof(int)*length); //выделение пам€ти под массив, который будет отсортирован сортировкой пузырьком
        int *massHeap=malloc(sizeof(int)*length); //выделение пам€ти под массив, который будет отсортирован пиромидальной сортировкой
        generation(massBubble, massHeap, length);
        timeBubble=bubbleSort(massBubble, length); //сортировка массива пузырьком, в переменную timeBubble заноситс€ врем€ работы алгоритма
        timeHeap=heapSort(massHeap, length); //пиромидальна€ сортировка массива, в переменную timeBubble заноситс€ врем€ работы алгоритма
        fprintf(file, "%d %f %f\n", length, timeBubble, timeHeap); //запись полученных данных в файл
        printf("%d\n", length);
        free(massBubble); //высвобождение выделенной дл€ массива пам€ти
        free(massHeap); //высвобождение выделенной дл€ массива пам€ти
    }
